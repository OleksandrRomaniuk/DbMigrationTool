...
public enum MySqlQueriesConstants {
    ...
    TableConstraintsSelectAll("SELECT *  FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = '%s' AND  TABLE_NAME = '%s' AND  CONSTRAINT_NAME = '%s' ;"),
    KeyColumnUSageConstarintsSelectAll("SELECT *  FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE CONSTRAINT_SCHEMA = '%s' AND TABLE_NAME = '%s' AND COLUMN_NAME = '%s' ;");
